...
df_reference_model = pd.read_csv(getResultsFilePath(config, 'CCIS', False, False),header=0)
...
for alg in config['models']:
    ...
    dfs.append((df, df_reference_model, alg))
...
for j,ds in enumerate(datasets):
    ...
    for df, df_ref, alg in dfs:
        ...
        curves_IS = []
        df_tmp1 = df_reference_model[(df_reference_model['name']==ds).values]
...
save_results(results, f"summary_of_areas_under_curves_{config['result_postfix'][0]}.csv")
save_results(results_interp1d, f"summary_of_areas_under_curves_interp1d_{config['result_postfix'][0]}.csv")