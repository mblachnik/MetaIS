...
def trainMeta(fNames, Xs, ys, fN, config,is_model: str,meta_model):
    Xx = []
    yy = []
...
for model in config["models"]:
    files.extend([(r, f.replace(".csv",""), ".csv")
...
if n_jobs > 1: #We iterate over files so that for each file a separate dataset of metaattributes will be created, but from the full list of Xs and ys we have to droop current file becouse for that file we will perform experiments.
    Parallel(n_jobs=n_jobs, prefer="threads", backend="loky")(delayed(trainMeta)(fNames, Xs, ys, fN, config, multimodel, clone(meta_model)) for fN in tqdm(fNames))
else:
    for fN in tqdm(fNames):
        trainMeta(fNames, Xs, ys, fN, config, multimodel, clone(meta_model))
